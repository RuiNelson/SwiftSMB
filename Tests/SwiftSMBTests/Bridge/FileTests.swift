...
struct StatTests {
    @Test("stat known file returns file type") func statKnownFileReturnsFileType() throws {
        try withPublicShare { ctx in
            let stat = try Bridge.fileStatistics(context: ctx, path: TestContent.helloPath)
    ...
    @Test("stat known directory returns directory type") func statKnownDirectoryReturnsDirectoryType() throws {
        try withPublicShare { ctx in
            let stat = try Bridge.fileStatistics(context: ctx, path: TestContent.testdirPath)
    ...
    @Test("stat non existent path throws") func statNonExistentPathThrows() throws {
        try withPublicShare { ctx in
            #expect(throws: SMB.Error.self) {
                try Bridge.fileStatistics(context: ctx, path: "nonexistent_\(uniquePath())")
            }
        }
    }
    ...
    @Test("stat from handle matches stat from path") func statFromHandleMatchesStatFromPath() throws {
        try withPublicShare { ctx in
            let pathStat = try Bridge.fileStatistics(context: ctx, path: TestContent.helloPath)
            let handle = try Bridge.open(context: ctx, path: TestContent.helloPath)
            defer { try? Bridge.close(context: ctx, file: handle) }
            let handleStat = try Bridge.fileStatistics(context: ctx, file: handle)
    ...
    @Test("stat VFS has positive block size") func statVfsHasPositiveBlockSize() throws {
        try withPublicShare { ctx in
            let vfs = try Bridge.statVFS(context: ctx, path: "")
...
struct FileReadTests {
    @Test("open and close file") func openAndCloseFile() throws {
        try withPublicShare { ctx in
            let fh = try Bridge.open(context: ctx, path: TestContent.helloPath)
            try Bridge.close(context: ctx, file: fh)
        }
    }
    ...
    @Test("read nested file content") func readNestedFileContent() throws {
        try withPublicShare { ctx in
            let fh = try Bridge.open(context: ctx, path: TestContent.nestedPath)
            defer { try? Bridge.close(context: ctx, file: fh) }
            let bytes = try readAllBytes(context: ctx, file: fh)
    ...
    @Test("seek set and read") func seekSetAndRead() throws {
        try withPublicShare { ctx in
            ...
            let newOffset = try Bridge.seek(context: ctx, file: fh, offset: 7, whence: SEEK_SET)
            #expect(newOffset == 7)
    ...
    @Test("seek cur advances position") func seekCurAdvancesPosition() throws {
        try withPublicShare { ctx in
            ...
            let offset = try Bridge.seek(context: ctx, file: fh, offset: 3, whence: SEEK_CUR)
            #expect(offset == 3)
        }
    }
    ...
    @Test("seek end positions at end of file") func seekEndPositionsAtEndOfFile() throws {
        try withPublicShare { ctx in
            ...
            let offset = try Bridge.seek(context: ctx, file: fh, offset: 0, whence: SEEK_END)
    ...
    @Test("open non existent file throws") func openNonExistentFileThrows() throws {
        try withPublicShare { ctx in
            #expect(throws: SMB.Error.self) {
                try Bridge.open(context: ctx, path: "nonexistent_\(uniquePath()).txt")
            }
        }
    }
    ...
    @Test("private share file is readable") func privateShareFileIsReadable() throws {
        try withPrivateShare { ctx in
            let fh = try Bridge.open(context: ctx, path: "secret.txt")
            defer { try? Bridge.close(context: ctx, file: fh) }
            let bytes = try readAllBytes(context: ctx, file: fh)
            #expect(!bytes.isEmpty)
        }
...
struct FileWriteTests {
    @Test("create write read and delete file") func createWriteReadAndDeleteFile() throws {
        try withPublicShare { ctx in
            ...
            let wh = try Bridge.open(
                context: ctx,
                path: path,
                flags: Bridge.SMB2OpenFlags(.writeOnly, options: [.create, .exclusive]),
            )
    ...
    @Test("truncate by path shortens file") func truncateByPathShortensFile() throws {
        try withPublicShare { ctx in
            ...
            try Bridge.truncate(context: ctx, path: path, length: 5)
    ...
    @Test("rename file") func renameFile() throws {
        try withPublicShare { ctx in
            ...
            defer {
                try? Bridge.unlink(context: ctx, path: oldPath)
                try? Bridge.unlink(context: ctx, path: newPath)
            }
            ...
            try Bridge.rename(context: ctx, oldPath: oldPath, newPath: newPath)

            #expect(throws: SMB.Error.self) {
                try Bridge.fileStatistics(context: ctx, path: oldPath)
            }

            let stat = try Bridge.fileStatistics(context: ctx, path: newPath)
            #expect(stat.type == .file)
        }
    }
    ...
    @Test("sync file succeeds") func syncFileSucceeds() throws {
        try withPublicShare { ctx in
            ...
            try Bridge.sync(context: ctx, file: wh)
        }
    }
    ...
    @Test("unlink removes file") func unlinkRemovesFile() throws {
        try withPublicShare { ctx in
            ...
            try Bridge.unlink(context: ctx, path: path)
    ...
    @Test("unlink non existent file throws") func unlinkNonExistentFileThrows() throws {
        try withPublicShare { ctx in
            #expect(throws: SMB.Error.self) {
                try Bridge.unlink(context: ctx, path: "nonexistent_\(uniquePath()).txt")
            }
        }
    }
        ...
        try withReadonlyShare { ctx in
            #expect(throws: SMB.Error.self) {
                try Bridge.open(
                    ...
                    flags: Bridge.SMB2OpenFlags(.writeOnly, options: [.create]),
                )
            }
        }
    }
...
struct SymlinkTests {
    @Test("read link for file symlink") func readLinkForFileSymlink() throws {
        try withPublicShare { ctx in
            let target = try Bridge.readLink(context: ctx, path: TestContent.linkToFilePath)
    ...
    @Test("read link for directory symlink") func readLinkForDirectorySymlink() throws {
        try withPublicShare { ctx in
            let target = try Bridge.readLink(context: ctx, path: TestContent.linkToDirPath)
            #expect(!target.isEmpty)
        }
    }
    ...
    @Test("read link on regular file throws") func readLinkOnRegularFileThrows() throws {
        try withPublicShare { ctx in
            #expect(throws: SMB.Error.self) {
                try Bridge.readLink(context: ctx, path: TestContent.helloPath)
            }
        }
    }
...
struct ReadonlyShareTests {
    @Test("read file from readonly share") func readFileFromReadonlyShare() throws {
        try withReadonlyShare { ctx in
            let fh = try Bridge.open(context: ctx, path: "readme.txt")
            defer { try? Bridge.close(context: ctx, file: fh) }
            let bytes = try readAllBytes(context: ctx, file: fh)
    ...
    @Test("stat from readonly share") func statFromReadonlyShare() throws {
        try withReadonlyShare { ctx in
            let stat = try Bridge.fileStatistics(context: ctx, path: "readme.txt")
            #expect(stat.type == .file)
            #expect(stat.size > 0)
        }
    }
    ...
    @Test("open file readWrite and write read back") func openFileReadwriteAndWriteReadBack() throws {
        try withPublicShare { ctx in
            ...
            let wh = try Bridge.open(
                context: ctx,
                path: path,
                flags: Bridge.SMB2OpenFlags(.readWrite, options: [.create, .exclusive]),
            )
            let content = Array("read-write test".utf8)
            _ = try writeAllBytes(context: ctx, file: wh, data: content)

            _ = try Bridge.seek(context: ctx, file: wh, offset: 0, whence: SEEK_SET)
...
struct LargeFileTests {
    @Test("largefile 5 MB is on private share") func largefile5MbIsOnPrivateShare() throws {
        try withPrivateShare { ctx in
            let stat = try Bridge.fileStatistics(context: ctx, path: "largefile.bin")
...
struct SetBasicInfoTests {
    @Test("set modification time") func setModificationTime() throws {
        try withPublicShare { ctx in
            ...
            try Bridge.setStats(context: ctx, path: path, lastWriteTime: newTime)
    ...
    @Test("set access time") func setAccessTime() throws {
        try withPublicShare { ctx in
            ...
            try Bridge.setStats(context: ctx, path: path, lastAccessTime: newTime)
    ...
    @Test("set creation time") func setCreationTime() throws {
        try withPublicShare { ctx in
            ...
            try Bridge.setStats(context: ctx, path: path, creationTime: newTime)
    ...
    @Test("get and set file attributes") func getAndSetFileAttributes() throws {
        try withPublicShare { ctx in
            ...
            let initial = try Bridge.getFileAttributes(context: ctx, path: path)
            #expect(initial == 0x0000_0020) // SMB2_FILE_ATTRIBUTE_ARCHIVE

            try Bridge.setStats(context: ctx, path: path, fileAttributes: 0x0000_0002)

            let updated = try Bridge.getFileAttributes(context: ctx, path: path)
