...
``SMB.Connection.copyFile(from:to:)`` copies a file directly on the server without sending its data through the client.
This is especially efficient for large files. The server must support server-side copies.
