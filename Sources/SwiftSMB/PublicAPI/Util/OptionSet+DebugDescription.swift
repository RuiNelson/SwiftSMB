...
extension OptionSet where Self == Self.Element {
    /// Returns a debug description listing the names of the flags present in
    /// this set, or `[]` when empty.
    ///
    /// - Parameters:
    ///   - flags: A list of known flag values paired with their names, in
    ///     the order they should appear in the description.
...
extension CustomDebugStringConvertible {
    ...
    ///
    /// Used for raw values in ``unknown``-style enum cases, for example
    /// `SMB.NodeType.unknown(0xC000_0034)`.
